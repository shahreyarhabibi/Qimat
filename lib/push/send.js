...
function getVapidConfig() {
  ...
  const privateKey = String(process.env.VAPID_PRIVATE_KEY || "")
    .replace(/\s+/g, "")
    .trim();
  let subject = String(process.env.VAPID_SUBJECT || "").trim();

  if (!publicKey || !privateKey || !subject) {
    return null;
  }

  // Be tolerant with env input to avoid silently breaking notifications.
  if (!subject.startsWith("mailto:") && !subject.startsWith("https://")) {
    if (subject.includes("@")) {
      subject = `mailto:${subject}`;
    } else {
      subject = "mailto:no-reply@qimat.local";
    }
  }
...
export async function sendPriceChangePush({
  ...
  const body = `${newPrice} (${isIncrease ? "+" : "-"}${absChange}) ${currencyLabel}`;
