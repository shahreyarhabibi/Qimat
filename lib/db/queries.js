...
export async function getProductsWithPrices() {
  ...
  const [
    ...
    // Query 3: All prices for last 30 days (we'll filter in JS)
    db
      .select({
      ...
      .from(prices)
      .where(and(gte(prices.date, thirtyDaysAgo), lte(prices.date, today)))
      .orderBy(prices.productId, asc(prices.date)),

    // Query 4: All calculator configs
    db.select().from(calculatorConfigs),

    // Query 5: All presets
    db.select().from(calculatorPresets).orderBy(calculatorPresets.sortOrder),

    // Query 6: All price units
    db.select().from(priceUnits).orderBy(priceUnits.sortOrder),
  ]);

  if (productsData.length === 0) return [];

  // Build lookup maps (this is fast, runs in memory)
  const sourcesMap = {};
  sourcesData.forEach((s) => {
    sourcesMap[s.slug] = {
      ...
      namePs: s.namePs,
      shortName: s.shortName,
      shortNameFa: s.shortNameFa,
      shortNamePs: s.shortNamePs,
    };
  });
  ...
  const mappedProducts = productsData.map((product) => {
    ...
    const weekAgoPrice =
      priceHistory.find((p) => p.date === weekAgo)?.price || todayPrice;
    const change = Math.round((todayPrice - weekAgoPrice) * 100) / 100;
    ...
    return {
      ...
      namePs: product.namePs,
      slug: product.slug,
      unit: product.unit,
      unitFa: product.unitFa,
      unitPs: product.unitPs,
      price: todayPrice,
      change: change,
      ...
      descriptionPs: product.descriptionPs || "",
      priceHistory: priceHistory,
      calculator: config
        ? {
            baseQuantity: config.baseQuantity,
            defaultQuantity: config.defaultQuantity,
            displayUnit: config.displayUnit || product.unit || "unit",
            step: config.step,
  ...
  const result = mappedProducts.sort(
...
export async function getTickerItems() {
  ...
  return result.map((product) => {
    ...
    const weekAgoPrice =
      productPrices.find((p) => p.date === weekAgo)?.price || currentPrice;
