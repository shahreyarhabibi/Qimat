...
export async function createProduct(data) {
  ...
  const productResult = await client.execute({
    sql: `
      INSERT INTO products (name, slug, description, category_slug, unit, image_path, source_slug, is_active, is_featured)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `,
    args: [
      data.name,
...
export async function updatePrice(productId, price, date) {
  ...
  const existing = await client.execute({
    sql: `SELECT id FROM prices WHERE product_id = ? AND date = ?`,
...
export async function createCategory(data) {
  ...
  const result = await client.execute({
    sql: `INSERT INTO categories (slug, name, icon, sort_order) VALUES (?, ?, ?, ?)`,
    args: [slug, data.name, data.icon || null, sortOrder],
  });
...
export async function createSource(data) {
  ...
  const result = await client.execute({
    sql: `INSERT INTO sources (slug, name, short_name) VALUES (?, ?, ?)`,
    args: [slug, data.name, data.shortName || data.name],
  });
