...
export async function getAllProducts() {
  ...
  const result = await client.execute(`
    ...
    latest_prices AS (
      SELECT pr.product_id, pr.price as current_price, lpd.last_price_date
      FROM latest_price_dates lpd
      JOIN prices pr
        ON pr.product_id = lpd.product_id
       AND pr.date = lpd.last_price_date
    ),
    price_counts AS (
    ...
    LEFT JOIN price_counts pc ON pc.product_id = p.id
    ORDER BY p.id DESC
  `);
...
export async function getAnalytics() {
  ...
  const [
    ...
    client.execute(`SELECT COUNT(*) as count FROM products WHERE is_active = 1`),
    client.execute(`
      SELECT
        c.name,
        c.slug,
    ...
    client.execute(`
      ...
      JOIN prices pr
        ON pr.product_id = lpd.product_id
       AND pr.date = lpd.latest_date
      JOIN products p ON p.id = lpd.product_id
    ...
    client.execute(`
      ...
      ORDER BY date DESC
    `),
    client.execute(`SELECT COUNT(*) as count FROM visits`),
    client.execute({
